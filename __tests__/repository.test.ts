...
test('corrupt storage falls back gracefully instead of throwing', async () => {
  // Persist raw corrupt strings that JSON.parse cannot handle.
  await AsyncStorage.setItem('pulse.settings', '{bad json');
  await AsyncStorage.setItem('pulse.history', '{bad json');
  const settings = await loadSettings();
  expect(settings.units).toBe('km');
  expect(settings.weeklyGoalKm).toBe(60);
  expect(settings.lastDeviceId).toBeNull();
